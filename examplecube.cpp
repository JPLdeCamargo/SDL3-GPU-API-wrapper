...
static SDL_GPUTexture *CreateResolveTexture(Uint32 drawablew,
                                            Uint32 drawableh) {
    SDL_GPUTextureCreateInfo createinfo;
    SDL_GPUTexture *result;
...
static void Render(SDL_Window *window, const int windownum) {
    ...
    if (!SDL_AcquireGPUSwapchainTexture(cmd, state->windows[windownum],
                                        &swapchainTexture, &drawablew,
                                        &drawableh)) {
    ...
    if (winstate->prev_drawablew != drawablew ||
...
static void init_render_state(int msaa) {
    ...
    gpu_device =
        SDL_CreateGPUDevice(TESTGPU_SUPPORTED_FORMATS, true, state->gpudriver);
    ...
    buf_transfer =
        SDL_CreateGPUTransferBuffer(gpu_device, &transfer_buffer_desc);
    ...
    pipelinedesc.target_info.depth_stencil_format =
    ...
    pipelinedesc.depth_stencil_state.compare_op =
    ...
    pipelinedesc.vertex_input_state.vertex_buffer_descriptions =
        &vertex_buffer_desc;
    pipelinedesc.vertex_input_state.num_vertex_attributes = 2;
    pipelinedesc.vertex_input_state.vertex_attributes =
    ...
    for (i = 0; i < state->num_windows; i++) {
        ...
        SDL_GetWindowSizeInPixels(state->windows[i], (int *)&drawablew,
                                  (int *)&drawableh);
        winstate->tex_depth = CreateDepthTexture(drawablew, drawableh);
        winstate->tex_msaa = CreateMSAATexture(drawablew, drawableh);
        winstate->tex_resolve = CreateResolveTexture(drawablew, drawableh);
...
int main(int argc, char *argv[]) {
    ...
    mode =
        SDL_GetCurrentDisplayMode(SDL_GetDisplayForWindow(state->windows[0]));
